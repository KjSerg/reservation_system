...
$(document).ready(function () {
  ...
  $(".form_send").not('#new_review').each(function () {
    var it = $(this);
    it.validate({
      rules: {
        phone: {
      ...
      submitHandler: function submitHandler(form) {
        ...
        if(thisForm.find('.mfv_checker input').length) {
            if (thisForm.find('.mfv_checker input').is(':checked')) {
                $.ajax({
                  ...
                  if(it.hasClass('create_new_order--js')) {

                    if(r) {
                      var req = JSON.parse(r);

                      if(req.type == 'failed') {

                        window.location.href = it.find('.redirect-f').val();

                      }else {
                          window.location.href = it.find('.redirect-s').val();
                      }
                    }
  ...
  $(".calendar_select select").on('selectric-init', function () {
    $(this).closest(".select_wrapper").addClass('changed').attr("data-value", $(this).find('option:selected').text());
          ...
          $.ajax({
              ...
              success: function(r) {
                ...
                var req = JSON.parse(r);
                  ...
                  var result = req.filter(function (x) {
                      return x.month === key;
                  });
